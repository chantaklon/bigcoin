...
#include "ui_zbigcontroldialog.h"
...
std::list<std::string> ZBIGControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBIGControlDialog::listMints;

ZBIGControlDialog::ZBIGControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBIGControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBIGControlDialog::~ZBIGControlDialog()
{
    delete ui;
}

void ZBIGControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBIGControlDialog::updateList()
{
...
void ZBIGControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBIGControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBIG_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBIGControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBIGControlDialog::GetSelectedMints()
{
...
void ZBIGControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
