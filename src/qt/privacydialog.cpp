                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBIG ought to be enough for anybody." - Bill Gates, 2017
    ui->zBIGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBIGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBIG->setEnabled(false);
        ui->pushButtonMintzBIG->setToolTip(tr("zBIG is currently disabled due to maintenance."));

        ui->pushButtonSpendzBIG->setEnabled(false);
        ui->pushButtonSpendzBIG->setToolTip(tr("zBIG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBIGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBIG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBIG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBIG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBIG in ") +
...
void PrivacyDialog::on_pushButtonSpendzBIG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBIG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBIG
        sendzBIG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBIG
    sendzBIG();
}

void PrivacyDialog::on_pushButtonZBIGControl_clicked()
{
    ZBIGControlDialog* zBIGControl = new ZBIGControlDialog(this);
    zBIGControl->setModel(walletModel);
    zBIGControl->exec();
}

void PrivacyDialog::setZBIGControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBIGSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBIG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BIG Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBIGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBIG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BIG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBIG</b>";
    ...
    // use mints from zBIG selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBIGControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBIGControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbig selector in case it was used
    ZBIGControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBIG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BIG, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBIG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBIG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBIG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBIG "));
    ui->labelzBIGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBIGSyncStatus->setVisible(fShow);
}
